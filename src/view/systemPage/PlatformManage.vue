  ...
  <div>
    ...
    <el-dialog title="新增平台" :visible.sync="dialogNewPlatform" :modal=false @close="closeNewPlatformDialog">
      <el-form :model="formNewPlatform" :rules="platformRules" ref="formNewPlatform">
        <el-form-item label="新增平台名称" :label-width="formLabelWidth" prop="platformName">
          <el-input v-model="formNewPlatform.platformName" style="width: 80%"></el-input>
        </el-form-item>
        <el-form-item label="新增平台信息" :label-width="formLabelWidth" prop="platformDetail">
    ...
    cID2() {
      ...
      this.$axios.post('/user/changePlatformDetail',params).then(res => {
        if(res.data.code===1){
          ...
          // 表格数据修改,仅适用于较少数据
          let item;
          for(item in this.platformTableData){
            if(this.platformTableData[item].platformId===this.formChangePlatformInfo.platformId){
              this.platformTableData[item].platformDetail=this.formChangePlatformInfo.newPlatformInfo;
              break;
            }
          }
